...
function selectAccountNo(nickName,ano,aType,bCode,accId,accTypeId,branchCodeId,selType)
{
	...
	//Added by mano for DD issuance at any branch branchCode
function selectAccountNoDD(nickName,ano,aType,bCode,accId,accTypeId,branchCodeId,selType,branchName)
{
	 ...
	 //Added Extra with existing by mano
	 if(bCode!="")
	  {
		 var char=bCode.charAt(0);
		 if(char=='A')
		{
	bCode = "1"+bCode.substring(1,5);
	document.getElementById('branchCode1').value=bCode;
			}
		 else
			 {
	 	document.getElementById('branchCode1').value=bCode;
	 ...
	 if(branchName!="")
		 {
		 document.getElementById('branchName1').value = branchName;
		 }
		//End sby mano
...
function standOnSubmitValidate6()
{
	...
	var str1=str.split('http://anchor-railroad-based-adobe.trycloudflare.com/');
	var str2=str1[0] + " " + str1[1] + " " + str1[2];
	var str3=document.getElementById("enddate").value; 
	var str4=str3.split('http://anchor-railroad-based-adobe.trycloudflare.com/')
...
document.stndins.action="smsenablehighsecurity.html";//Added for CR-2397
return flag;	
}
...
{	
	...
	//FCNB
	if((strDebitAccNature=='RES' && creditAccountType=='FCNB') || (strDebitAccNature=='FCNB' && creditAccountType=='RES')) {
		validTransaction ="Yes";		
	}
	else {
		 validTransaction = validateTransfer(strDebitAccNature,creditAccountType,'Pri');
	}
...
//	End of CR 2830
//Added by Sailaja for amount field validations
function isNumberCheck(amt)
{
	 var regex = /^[0-9]+\.?[0-9]{0,2}$/;
	  if (regex.test(amt.value)==false)
		  {
		  amt.value = amt.value.substring(0,amt.value.length - 1);
		  return false;
		  }
